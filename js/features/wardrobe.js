function showItemTooltip(event, item, unequipConfig = null) {
    ...
    if (unequipConfig) {
        ...
        unequipBtn.onclick = () => {
            ...
            buildWardrobe(); // Refresh wardrobe UI
            updateCharDesc(); // Refresh character description
    ...
    positionTooltipForWardrobe(tooltip, event.target);
}

function positionTooltipForWardrobe(tooltipEl, targetEl) {
    const parentOverlay = document.getElementById('wardrobe-overlay');
...
function openItemSelectionOverlay(event, slotId, subSlot = null) {
    ...
    positionTooltipForWardrobe(subOverlay, event.target);
}
...
`;
    ...
    const createSlotWidget = (slotId) => {
        ...
        nameDiv.className = 'slot-name'; // Add a class for easier selection
